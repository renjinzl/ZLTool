...
open class HoverItemDecoration : RecyclerView.ItemDecoration() {
    ...
    val cancelEvent = Runnable {
        overViewHolder?.apply {
            ...
            itemView.dispatchTouchEvent(
                MotionEvent.obtain(nowTime(), nowTime(), MotionEvent.ACTION_UP, 0f, 0f, 0)
            )
        }
    }
    ...
    private fun addHoverView(view: View) {
        if (view.parent == null) {
            windowContent?.addView(view, 0, FrameLayout.LayoutParams(overDecorationRect.width(), overDecorationRect.height()).apply {
                ...
                parent.adapter?.let { adapter ->
                    hoverCallback?.let { callback ->
                        ...
                        if (firstChildHaveOver) {
                            ...
                            //创建第一个位置的child 需要分割线
                            val firstViewHolder = callback.createDecorationOverView.invoke(parent, adapter, firstChildHaveOverPosition)
                            ...
                            if (nextViewHolder != null) {
                                ...
                                if (callback.haveOverDecoration.invoke(adapter, nextViewHolder.adapterPosition) && !callback.isOverDecorationSame.invoke(adapter, firstChildAdapterPosition, nextViewHolder.adapterPosition)) {
                                    //不同的分割线, 实现上推效果
                                    if (nextViewHolder.itemView.top < tempRect.height()) {
                                        tempRect.offsetTo(0, nextViewHolder.itemView.top - tempRect.height())
                                    }
                                }
                            }
     ...
     * */
    internal fun findOverStartPosition(adapter: RecyclerView.Adapter<RecyclerView.ViewHolder>, adapterPosition: Int): Int {
    ...
    class HoverCallback {
         ...
         * 当前的 位置 是否有 悬浮分割线
         * */
        var haveOverDecoration: (adapter: RecyclerView.Adapter<*>, adapterPosition: Int) -> Boolean = { adapter, adapterPosition ->
...
//                    adapter.getItemData(adapterPosition).itemIsHover
         ...
         * */
        var decorationOverLayoutType: (adapter: RecyclerView.Adapter<*>, adapterPosition: Int) -> Int = { adapter, adapterPosition ->
            if (adapter is ZLRecyclerViewAdapter) {
         ...
         * */
        var isOverDecorationSame: (adapter: RecyclerView.Adapter<RecyclerView.ViewHolder>, nowAdapterPosition: Int, nextAdapterPosition: Int) -> Boolean = { _, _, _ ->
            false
        }
         ...
         * 创建 分割线 视图
         * */
        var createDecorationOverView: (recyclerView: RecyclerView, adapter: RecyclerView.Adapter<RecyclerView.ViewHolder>, overAdapterPosition: Int) -> RecyclerView.ViewHolder = { recyclerView, adapter, overAdapterPosition ->
...
//        }
         ...
         * */
        var drawOverDecoration: (canvas: Canvas, paint: Paint, viewHolder: RecyclerView.ViewHolder, overRect: Rect) -> Unit = { canvas, paint, viewHolder, overRect ->
         ...
         * */
        var drawOverShadowDecoration: (canvas: Canvas, paint: Paint, viewHolder: RecyclerView.ViewHolder, overRect: Rect) -> Unit = { canvas, paint, viewHolder, overRect ->
